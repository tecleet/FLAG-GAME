...
A real-time YouTube Live interactive game where viewers guess mystery flags using emojis or country names. Fully automated with a Cyberpunk 2077 aesthetic.
...
- **Interactive Settings**: Configure Round Duration, Blur Difficulty, Stream Latency, and Mock Mode via the "CONFIG" button in the HUD.
- **Latency Compensation**: Adjustable buffer (default 25s) to ensure viewers have time to guess despite stream delay.
- **Flexible Guessing**: Accepts both **Flag Emojis** (🇺🇸) and **Country Names** ("United States", "USA", "America").
...
5. Set **STREAM LATENCY** to match your stream delay (default is 25s for YouTube).
6. Uncheck **MOCK MODE**.
7. Click **APPLY CHANGES**.

The system will connect to the chat and listen for flag emojis and text guesses.

## Game Rules

1. A blurred flag appears on screen.
2. Viewers must type the correct **Flag Emoji** or **Country Name** in the chat.
